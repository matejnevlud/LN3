...
import numpy as np
import time
from config import W, DETECTION_BASIC_RADIUS, DETECTION_COOLDOWN, IS_DEBUG, DETECTION_CIRCLE_PADDING, DETECTION_SQUARE_X_OFFSETS, DETECTION_CIRCLE_SENSITIVITY, DETECTION_SQUARE_SENSITIVITY, DETECTION_SQUARE_PADDING, NUMBER_OF_REGIONS, DETECTION_CIRCLE_X_OFFSETS
...
class DetectionCircle:
    ...
    def init_points(database):
        detection_points = []
        for i in range(0, NUMBER_OF_REGIONS):
            x = int(W / NUMBER_OF_REGIONS * i) + DETECTION_CIRCLE_PADDING + DETECTION_CIRCLE_X_OFFSETS[i]
            y = 50 + DETECTION_CIRCLE_PADDING
            d = W//NUMBER_OF_REGIONS - 2 * DETECTION_CIRCLE_PADDING
            r = d // 2
...
class DetectionSquare:
    ...
    def update(self, mask, is_moving):
        focused_region = mask[self.y:self.y+self.w, self.x:self.x+self.w]
    ...
    def draw(self, frame, is_moving):
        if not is_moving:
            cv2.rectangle(frame, (self.x, self.y), (self.x+self.w, self.y+self.w), (0, 140, 255), -1)
        elif self.is_detected:
            cv2.rectangle(frame, (self.x, self.y), (self.x+self.w, self.y+self.w), (0, 255, 0), -1)
        else:
            cv2.rectangle(frame, (self.x, self.y), (self.x+self.w, self.y+self.w), (0, 0, 255), -1)
